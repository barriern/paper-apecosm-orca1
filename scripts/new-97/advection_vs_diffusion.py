...
for l in [3, 20, 90]:
    ...
    toplot = (wstep * (zadv + madv + zdiff + mdiff)).sel(l=l, method='nearest')
    ...
    cpt += 1
    ax = axgr[cpt]
    toplot = (wstep * (zadv + madv)).cumsum(dim='time').sel(l=l, method='nearest')
    toplot = (wstep * (zadv + madv)).sel(l=l, method='nearest')
    cs, cl = plot(ax, toplot.values, 1, clim=ccc)
    cb = plt.colorbar(cs, cax=axgr.cbar_axes[cpt])
    ax.text(lon1, time1, letters[cpt], **textprop)
    ax.text(lon2, time2, '%scm' %l, **textprop)
    ...
    cpt += 1
    ax = axgr[cpt]
    toplot = (wstep * (zdiff + mdiff)).cumsum(dim='time').sel(l=l, method='nearest')
    toplot = (wstep * (zdiff + mdiff)).sel(l=l, method='nearest')
