...
mesh = xr.open_dataset('../fig6/equatorial_mesh_mask.nc')
...
wstep2 = wstep.sel(l=[3, 2, 90], method='nearest')
wstep
...
def read_variable(varname):
    ...
    if varname == 'OOPE':
        ...
        data['l'] = const['l'].sel(l=[3, 20, 90], method='nearest')
    return data
...
#axgr = ImageGrid(fig, 111, ngrids=5, nrows_ncols=(2, 3), axes_pad=(0.7, 0.4), cbar_pad=0.1, direction='row', aspect=False, cbar_mode="each", share_all=True, label_mode='L')
...
toplot = (wstep2 * (growth + pred)).isel(l=lindex)
...
cpt += 1 
ax = plt.subplot(2, 3, cpt)
toplot = (wstep2 * (growth)).isel(l=lindex)
cs, cl = plot(ax, toplot.values, 1, clim=ccc, contour=0)
cb = plt.colorbar(cs, )
ax.text(lon1, time1, letters[cpt - 1], **textprop)
...
cpt += 1
ax = plt.subplot(2, 3, cpt)
toplot = (wstep2 * (pred)).isel(l=lindex)
