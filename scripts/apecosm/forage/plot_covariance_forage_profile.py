...
data = xr.open_dataset("data/profile_covariance_yearly_enso.nc")
data = data.isel(community=0, depth=idepth)
data['x'] = lon
data = data.sortby(data['x'])
lon = data['x'].values
forage = data['covariance'].to_masked_array()
forage = np.squeeze(forage)

ilon = np.nonzero((lon <= 300) & (lon >= 130))[0]

lon = lon[ilon]
forage = forage[:, ilon, :, :] # dn, x, depth, size
...
for i, ax in enumerate(axgrid):
    ...
    cmax = np.percentile(np.abs(temp[itemp]), 99)
    cmin = -cmax

    cs = ax.pcolormesh(lon, -depth, temp, cmap=plt.cm.RdBu_r, shading='auto')
    cs.set_clim(cmin, cmax)
    cb = cax[i].colorbar(cs)
    #cb = mpl.colorbar.Colorbar(cax, cs)
    cb.set_label('$J.m^{-3}$')
...
plt.savefig('forage_covariance.png', bbox_inches='tight')
